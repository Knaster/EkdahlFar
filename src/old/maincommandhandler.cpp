...
void processCommand(commandItem *inCommandItem, std::vector<commandResponse> *commandResponses, bool delegated = false, commandList *delegatedCommands = nullptr);
void processCommandList(commandList *inCommands, std::vector<commandResponse> *commandResponses);
void dir(std::vector<commandResponse> *commandResponses, String longPrefix, String shortPrefix);
...
/*    if (inCommandItem->command == "modulecount") {
    ...
    if (inCommandItem->command == "dir") {
        dir(commandResponses, "", "");
    } else
    ...
    if (inCommandItem->command == "globaluservariable") {
        ...
        commandResponses->push_back({"guv:" + String(userVariable) + ":" + String(expressionParser.duv[userVariable]), InfoRequest});
    } else
    ...
    if (inCommandItem->command == "midiconfiguration") {
        if (request) {
            ...
            while (currentConfig > (configArray.size() - 1)) {
                configArray.push_back(MIDIMessageConfiguration());
...
//            farSingle->midiHandler->midiAllNotesOff();
            commandResponses->push_back({ "mano:1", debugPrintType::InfoRequest});
        }
    } else
    if (inCommandItem->command == "ifequal") {
        if (!request) {
            if (!checkArguments(inCommandItem, commandResponses, 4)) { return eProcessResult::WrongArgumentCount; }
            ...
            processCommandList(executeNow, commandResponses);
        }
    } else
    if (inCommandItem->command == "ifgreater") {
        if (!request) {
            if (!checkArguments(inCommandItem, commandResponses, 4)) { return eProcessResult::WrongArgumentCount; }
            commandList *executeNow;
            if (inCommandItem->argument[0].toInt() > inCommandItem->argument[1].toInt()) {
    ...
    if (inCommandItem->command == "ifless") {
        if (!request) {
            if (!checkArguments(inCommandItem, commandResponses, 4)) { return eProcessResult::WrongArgumentCount; }
            commandList *executeNow;
            if (inCommandItem->argument[0].toInt() < inCommandItem->argument[1].toInt()) {
    ...
    if (inCommandItem->command == "expressionparserevaluate") {
        if (!checkArguments(inCommandItem, commandResponses, 1)) { return eProcessResult::WrongArgumentCount; }
//        farSingle->midiHandler->updateLocalVariables();
        commandList testCommands("epev:" + inCommandItem->argument[0]);
        testCommands.parseCommandExpressions(expressionParser);
        commandResponses->push_back({ "epev: " + String(testCommands.item[0].argument[0]), debugPrintType::InfoRequest});
    } else
...
void processCommand(commandItem *inCommandItem, std::vector<commandResponse> *commandResponses, bool delegated = false, commandList *delegatedCommands = nullptr) {
        commandItem *tCommandItem = inCommandItem;

        eProcessResult processResult;
        bool request = false;

        if ((tCommandItem->command == "requestinfo") || (tCommandItem->command == "rqi")|| (tCommandItem->command == "?")) {
            if (checkArgumentsMin(tCommandItem, commandResponses, 1)) {
                String commandString = tCommandItem->argument[0].trim();
                for (int i = 0; i < tCommandItem->argument.size() - 1; i++) {
                    commandString += ":" + tCommandItem->argument[i];
                }
                //debugPrintln("Rebuilt RQI command as " + commandString, debugPrintType::Debug);
                tCommandItem = new commandItem(commandString);

/*                tCommandItem->command = tCommandItem->argument[0].trim();
                tCommandItem->argument.erase(tCommandItem->argument.begin());
                tCommandItem->rebuildCommand();*/
            } else {
        ...
        switch (farSingle->processCommands(tCommandItem, commandResponses, request)) {
        case eProcessResult::NotFound:
            debugPrintln("Command not found: " + tCommandItem->command, debugPrintType::Error);
            break;
        case eProcessResult::CommandFailed:
            debugPrintln("Command failed: " + tCommandItem->command, debugPrintType::Error);
            break;
        case eProcessResult::WrongArgumentCount:
        case eProcessResult::WrongArgumentMinimum:
            debugPrintln("Wrong command arguments: " + tCommandItem->command, debugPrintType::Error);
            break;
        case eProcessResult::WrongArgumentValue:
            debugPrintln("Wrong command argument values: " + tCommandItem->command, debugPrintType::Error);
            break;
        };
        return;
        ...
        if (tCommandItem->command == "help") {
            ...
            commandResponses->push_back({":\"Below is a list of avaliable commands given in both short and long form, either works. The number and type(s) of arguments are listed afterwards and a brief description. Command usage: command:argument1:argument2 [etc]\"", debugPrintType::TextInfo});
        ...
        if ((processResult == eProcessResult::NotFound) || (processResult == eProcessResult::PassThrough)) {
            processResult = farSingle->processSerialCommandHidden(tCommandItem, commandResponses, request);
        }

        if ((processResult == eProcessResult::NotFound) && (tCommandItem->command != "help") && (tCommandItem->command != "helpHidden")) {
...
void processCommandList(commandList *inCommands, std::vector<commandResponse> *commandResponses) {
    ...
    while (i < inCommands->item.size()) {
        processCommand(&(inCommands->item[i]), commandResponses);
        i++;
    }
...
void processSerialCommands() {
    ...
    processCommandList(commands, &commandResponses);
    commands->item.clear();
...
void dir(std::vector<commandResponse> *commandResponses, String longPrefix, String shortPrefix) {
    commandResponses->push_back({ "dir:'" + farSingle->identifier + "'", debugPrintType::InfoRequest });

    dirProto(commandResponses, longPrefix, shortPrefix, serialCommandsMain, sizeof(serialCommandsMain) / sizeof(serialCommandItem), nullptr, 0);

//    farSingle->dir(commandResponses, longPrefix, shortPrefix);
}
