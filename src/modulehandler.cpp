...
void ModuleHandler::help(std::vector<commandResponse> *inCommandResponses, String longPrefix, String shortPrefix) {
    String lp = longPrefix, sp = shortPrefix;

    if (lp != "") { if (lp[lp.length() - 1] != '.') { lp +=  "."; } }
    if (sp != "") { if (sp[sp.length() - 1] != '.') { sp +=  "."; } }
    Module::help(inCommandResponses, lp, sp);
    for (int i = 0; i < moduleGroups.size(); i++) {
        moduleGroups[i].help(inCommandResponses,lp + moduleGroups[i].tmoduleID.longName, sp + moduleGroups[i].tmoduleID.shortName);
    }
}
...
//            inCommandResponses->push_back({ "ins:" + moduleGroups[i].moduleID->getShortName() + ":" + String(moduleGroups[i].modules.size()), debugPrintType::InfoRequest });
            inCommandResponses->push_back({ "ins:" + String(moduleGroups[i].tmoduleID.shortName) + ":" + String(moduleGroups[i].modules.size()), debugPrintType::InfoRequest });
        }
...
//            commandResponse->push_back({ "ins:" + moduleGroups[i].moduleID->getShortName() + ":" + String(moduleGroups[i].modules.size()), debugPrintType::InfoRequest });
            commandResponse->push_back({ "ins:" + String(moduleGroups[i].tmoduleID.shortName) + ":" + String(moduleGroups[i].modules.size()), debugPrintType::InfoRequest });
        }
        return eProcessResult::Ok;
    }
...
//                (inCommandItem->hierarchy[inCommandItem->hierarchyIndex].name == moduleGroups[i].moduleID->getShortAlias())) {
            if ((inCommandItem->hierarchy[inCommandItem->hierarchyIndex].name == String(moduleGroups[i].tmoduleID.longName)) ||
                (inCommandItem->hierarchy[inCommandItem->hierarchyIndex].name == String(moduleGroups[i].tmoduleID.shortAlias.c_str()))) {
...
//                    String returnString = moduleGroups[i].moduleID->getShortAlias();
                    String returnString = moduleGroups[i].tmoduleID.shortAlias.c_str();
...
//        debugPrintln("Module " + inCommandItem->hierarchy[inCommandItem->hierarchyIndex].name + " not found in " + moduleID->getLongName() , debugPrintType::Error);
        debugPrintln("Module " + inCommandItem->hierarchy[inCommandItem->hierarchyIndex].name + " not found in " + getModuleID().longName , debugPrintType::Error);
    } else {
